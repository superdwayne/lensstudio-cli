...
def sample_project(tmp_dir):
    ...
    from cli_anything.lens_studio.core.project import create_project

    result = create_project("TestProject", directory=tmp_dir, template="blank")
    project_path = result["path"]
    project_dir = result["directory"]

    from cli_anything.lens_studio.core.project import load_project
    data = load_project(project_path)

    return data, project_path, project_dir
...
def sample_texture(tmp_dir):
    ...
    # Write minimal PNG header (1x1 pixel)
    import struct
    import zlib
