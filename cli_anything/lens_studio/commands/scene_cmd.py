...
def _load_project(ctx):
    """Load project and return (project_data, project_path)."""
...
def scene_list(ctx, json_mode):
    ...
    try:
        ...
        items = scene_core.flatten_scene(data)
        rows = []
        for item in items:
            indent = "  " * item["depth"]
...
def scene_tree(ctx, json_mode):
    ...
    try:
        data, path = _load_project(ctx)
        tree_data = scene_core.scene_to_tree(data)
...
def scene_add(ctx, name, parent, json_mode):
    ...
    try:
        ...
        parent_id = None
        if parent:
            obj = scene_core.find_object(data, parent)
            if not obj:
                obj = scene_core.find_object_by_name(data, parent)
            if obj:
                parent_id = obj["id"]
            else:
                error(f"Parent not found: {parent}", json_mode=json_mode)

        new_obj = scene_core.add_object(data, name, parent_id=parent_id)
...
def scene_remove(ctx, object_id, json_mode):
    ...
    try:
        data, path = _load_project(ctx)
        obj = scene_core.find_object(data, object_id)
        if not obj:
            obj = scene_core.find_object_by_name(data, object_id)
            if obj:
                object_id = obj["id"]
        scene_core.remove_object(data, object_id)
        proj_core.save_project(path, data)
...
def scene_rename(ctx, object_id, name, json_mode):
    ...
    try:
        data, path = _load_project(ctx)
        scene_core.rename_object(data, object_id, name)
        proj_core.save_project(path, data)
...
def scene_transform(ctx, object_id, position, rotation, scale, json_mode):
    ...
    try:
        ...
        obj = scene_core.set_transform(data, object_id, position=pos, rotation=rot, scale=scl)
        proj_core.save_project(path, data)
        success(
...
def scene_enable(ctx, object_id, off, json_mode):
    ...
    try:
        data, path = _load_project(ctx)
        enabled = not off
        scene_core.set_enabled(data, object_id, enabled)
        proj_core.save_project(path, data)
...
def scene_reparent(ctx, object_id, new_parent, json_mode):
    ...
    try:
        data, path = _load_project(ctx)
        scene_core.reparent(data, object_id, new_parent)
        proj_core.save_project(path, data)
...
def scene_duplicate(ctx, object_id, json_mode):
    ...
    try:
        data, path = _load_project(ctx)
        clone = scene_core.duplicate_object(data, object_id)
        proj_core.save_project(path, data)
        success(
